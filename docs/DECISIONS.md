...
| D-026 | The OrkaATS Apps Script boundary uses wire schema `v1` over the unchanged general adapter contract `1.0.0`. It remains disabled by default and unregistered in core composition; when explicitly configured it uses `httpx` and signs a canonical payload with a versioned key ID, UUID4 nonce, Unix timestamp, and HMAC-SHA-256. The endpoint and secret remain external configuration, and receiver replay defense and end-user identity are not inferred. | Makes the real transport locally mockable and request-authenticated without coupling core services or falsely claiming that connectivity, user identity, or production authorization is proven. |
